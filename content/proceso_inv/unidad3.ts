export const TEORIA = [
  {
    titulo: "1. Encontrar una fuente no significa haber encontrado la verdad",
    bloques: [{ tipo: "texto", contenido: `
En unidad 2 se aprendió a localizar y organizar fuentes pertinentes. Ahora corresponde dar un paso adicional: **someterlas a crítica**.

Una fuente no se convierte en verdad por ser antigua, oficial, académica o muy citada. Tampoco debe descartarse automáticamente porque sea interesada, posterior al hecho o contradictoria.

La pregunta metodológica fundamental es:

**¿Qué me permite saber esta fuente y con qué grado de confianza?**

Una misma fuente puede contener datos directamente comprobables, testimonios valiosos, interpretaciones discutibles, errores y silencios. Por ello, investigar no consiste en elegir a qué autor “creerle”, sino en determinar qué afirmaciones puede sostener la evidencia.

Dos actitudes deben evitarse:

- “Lo dice un libro, por lo tanto es cierto”.
- “Esta fuente tiene sesgo, por lo tanto no sirve”.

La crítica de fuentes exige una posición diferente: **interrogar antes de aceptar o rechazar**.
` }],
  },
  {
    titulo: "2. ¿Quién habla, cuándo y desde dónde?",
    bloques: [{ tipo: "texto", contenido: `
Toda fuente fue producida por alguien, en determinado momento y para algún propósito. Antes de utilizarla conviene reconstruir, en la medida de lo posible:

**autoría → fecha → proximidad → propósito → destinatario → contexto → alcance**

No se lee de la misma manera un decreto, una memoria oficial, un anuncio bancario, un editorial, una carta privada, el testimonio de un propietario de finca o un catálogo publicado décadas después.

La proximidad temporal puede ser valiosa, pero no garantiza exactitud. Un contemporáneo puede desconocer parte de lo sucedido o defender intereses concretos. Un investigador posterior puede disponer de archivos que el contemporáneo nunca conoció.

Preguntas útiles son:

1. ¿Quién produjo la fuente?
2. ¿Cuándo?
3. ¿Podía conocer directamente aquello que afirma?
4. ¿Para quién fue producida?
5. ¿Qué propósito tenía?
6. ¿Qué aspectos podía tener interés en destacar, ocultar o justificar?
7. ¿Hasta dónde alcanza realmente su testimonio?

**Conocer la posición de una fuente no sirve para desacreditarla, sino para utilizarla correctamente.**
` }],
  },
  {
    titulo: "3. Fuente primaria y secundaria: una relación, no una jerarquía",
    bloques: [{ tipo: "texto", contenido: `
La clasificación entre fuentes primarias y secundarias es útil, pero no debe convertirse en una escala automática de calidad.

Una **fuente primaria** mantiene una relación directa o contemporánea con el fenómeno estudiado: una pieza, un decreto, un registro de acuñación, una carta, un periódico de la época o un libro contable.

Una **fuente secundaria** analiza, organiza o interpreta evidencias producidas anteriormente.

Un decreto de 1924 puede ser fuente primaria para conocer qué ordenó el Estado. Un estudio publicado en 2026 es secundario respecto de aquel decreto, pero puede ser más adecuado para analizar un sistema histórico completo porque integra numerosas piezas y documentos.

Una fuente primaria puede equivocarse, exagerar, omitir o mentir. Una fuente secundaria puede realizar una reconstrucción rigurosa a partir de múltiples evidencias.

Por ello, la pregunta correcta no es:

**¿Cuál fuente es mejor?**

sino:

**¿Cuál fuente es adecuada para la afirmación que se intenta demostrar?**
` }],
  },
  {
    titulo: "4. Dependencia entre fuentes: diez autores pueden ser una sola voz",
    bloques: [{ tipo: "texto", contenido: `
La repetición bibliográfica puede producir una falsa sensación de certeza.

Suponga que un autor afirmó algo en 1950. Otro lo repitió en 1970. Un tercero citó al segundo. Posteriormente varios catálogos copiaron esa información.

Encontrar la misma afirmación en numerosos libros no significa necesariamente disponer de numerosas confirmaciones independientes. Puede existir una sola cadena:

**afirmación original → repetición → nueva repetición → tradición bibliográfica**

El investigador debe intentar construir la **genealogía de la afirmación**:

1. ¿Cuál es la referencia más antigua que logró localizar?
2. ¿Los autores posteriores aportan evidencia nueva?
3. ¿Citan directamente la fuente original?
4. ¿Se citan unos a otros?
5. ¿Puede localizarse el documento, pieza o evidencia en que se basó la afirmación inicial?

Una afirmación repetida durante décadas puede ser correcta. El problema es que **la repetición no constituye por sí misma corroboración**.

Rastrear el origen de una afirmación permite distinguir tradición bibliográfica de evidencia independiente.
` }],
  },
  {
    titulo: "5. El sesgo no invalida automáticamente una fuente",
    bloques: [{ tipo: "texto", contenido: `
Las fuentes son producidas por personas e instituciones situadas dentro de conflictos, intereses y estructuras sociales. El sesgo debe analizarse, no utilizarse como una etiqueta para descartar documentos.

Un testimonio favorable a determinado sistema de pago, producido por alguien que se beneficiaba de ese sistema, puede ser históricamente valioso. Sin embargo, no debería utilizarse como prueba neutral de sus efectos sin buscar evidencia independiente.

La investigación sobre fichas particulares ofrece un buen ejemplo metodológico. Testimonios de propietarios o actores vinculados con las fincas deben interpretarse considerando su posición dentro del propio sistema que describen (Morales Osterberg, 2026).

Las preguntas adecuadas son:

- ¿Qué interés podía tener quien produjo el documento?
- ¿Qué intentaba explicar, defender, justificar o conseguir?
- ¿Qué hechos podía conocer directamente?
- ¿Existe evidencia independiente que confirme su afirmación?
- ¿Hay voces o documentos procedentes de otros actores?

**Una fuente interesada no es una fuente inútil; es una fuente cuyo interés forma parte de la investigación.**
` }],
  },
  {
    titulo: "6. Contradicciones entre fuentes: el problema puede estar allí",
    bloques: [{ tipo: "texto", contenido: `
Cuando dos fuentes pertinentes parecen contradecirse, el investigador no debe escoger automáticamente la que parezca más prestigiosa.

Conviene:

1. comprobar que realmente hablan del mismo hecho;
2. verificar fechas y lugares;
3. identificar qué podía conocer cada autor;
4. determinar si una fuente depende de la otra;
5. examinar diferencias de vocabulario o significado;
6. buscar una tercera evidencia independiente.

A veces la contradicción se resuelve al descubrir que las fuentes se refieren a momentos distintos. Otras veces una fuente contiene un error. También puede ocurrir que la evidencia disponible no permita decidir.

En ese caso, una conclusión plenamente válida es:

**“La evidencia disponible no permite determinarlo con certeza.”**

Reconocer incertidumbre no representa un fracaso de la investigación. Es preferible declarar los límites de la evidencia que resolver artificialmente una contradicción.
` }],
  },
  {
    titulo: "7. El silencio de las fuentes",
    bloques: [{ tipo: "texto", contenido: `
En unidad 1 se introdujo una regla importante:

**ausencia de evidencia ≠ evidencia de ausencia.**

No encontrar una pieza o documento no demuestra automáticamente que nunca existió.

Sin embargo, el silencio puede adquirir valor cuando existe una razón sólida para esperar que determinado hecho hubiera dejado una huella.

Por ejemplo, si se conserva una serie sistemática y aparentemente completa de registros de fabricación para un período y una supuesta emisión no aparece en ellos, esa ausencia puede ser relevante. Pero deberá explicarse:

- por qué se esperaba encontrarla allí;
- qué tan completa es la serie documental;
- si existían otras vías de producción o registro;
- qué otras evidencias podrían confirmar o refutar la hipótesis.

La pregunta metodológica es:

**Si esto hubiera ocurrido, ¿qué huellas esperaríamos encontrar?**

Así, el silencio documental puede analizarse sin transformarlo arbitrariamente en prueba definitiva.
` }],
  },
  {
    titulo: "8. Evidencia, testimonio, inferencia e interpretación",
    bloques: [{ tipo: "texto", contenido: `
Conviene distinguir varios niveles en la construcción de una explicación histórica.

**Evidencia material:** existe una pieza con determinadas características.

**Testimonio documental:** un documento afirma que determinado hecho o práctica ocurrió.

**Inferencia:** la combinación de evidencias permite establecer razonablemente una relación que no aparece expresada de manera directa.

**Interpretación histórica:** se propone una explicación sobre el significado o función del fenómeno dentro de un contexto más amplio.

Una distinción especialmente importante es esta:

**Un documento que afirma algo constituye evidencia directa de que alguien hizo esa afirmación.**

Que lo afirmado sea además históricamente verdadero requiere evaluar el documento y, cuando sea necesario, contrastarlo con otras evidencias.

El investigador debe procurar que el lenguaje de sus conclusiones permita reconocer cuándo está describiendo una pieza, citando un testimonio, realizando una inferencia o proponiendo una interpretación.
` }],
  },
  {
    titulo: "9. ¿Cuándo puede construirse una afirmación sólida?",
    bloques: [{ tipo: "texto", contenido: `
La fortaleza de una afirmación depende de la correspondencia entre su alcance y la evidencia disponible.

Considere una emisión monetaria:

- un decreto puede documentar su autorización;
- un registro de fabricación puede documentar su producción;
- un ejemplar demuestra existencia material;
- un periódico puede anunciar su puesta en circulación;
- un registro comercial puede documentar utilización.

La convergencia de fuentes independientes puede construir una explicación robusta.

Pero no todas las afirmaciones necesitan la misma cantidad de evidencia.

Afirmar:

**“Existe al menos un ejemplar documentado con esta característica”**

requiere menos evidencia que afirmar:

**“Esta pieza circuló ampliamente en toda Guatemala.”**

Cuanto mayor sea la generalización, mayor deberá ser la evidencia que la respalde.

Por ello:

**la cantidad y diversidad de evidencia necesaria dependen del alcance de la afirmación.**

Una única fuente inequívoca puede resolver una pregunta muy concreta; una interpretación histórica amplia suele requerir convergencia documental.
` }],
  },
  {
    titulo: "10. La honestidad frente a la incertidumbre",
    bloques: [{ tipo: "texto", contenido: `
Una de las competencias fundamentales del Investigador AGENN será expresar con precisión el grado de certeza permitido por la evidencia.

Algunas expresiones útiles son:

- **demuestra:** la evidencia permite una conclusión directa;
- **documenta:** una fuente registra un hecho o una afirmación;
- **corrobora:** evidencia independiente converge con otra;
- **indica / sugiere:** existe evidencia razonable, pero insuficiente para certeza;
- **es compatible con:** la evidencia no contradice una hipótesis, aunque tampoco la demuestra;
- **podría:** se plantea una posibilidad;
- **no puede determinarse con la evidencia disponible:** la conclusión debe permanecer abierta.

Expresiones como “sin duda”, “indudablemente” o “es evidente” deben utilizarse únicamente cuando pueda justificarse semejante grado de certeza.

Modificar o abandonar una hipótesis frente a nueva evidencia no debilita al investigador. Por el contrario, demuestra que la conclusión depende de la evidencia y no del deseo de confirmar una idea previa.

**El objetivo de la investigación no es tener razón desde el principio, sino construir la explicación que mejor resista el examen de las fuentes.**
` }],
  },
  {
    titulo: "Actividad complementaria: Laboratorio de investigación 3 — ¿A quién le creo?",
    bloques: [{ tipo: "texto", contenido: `
Seleccione tres fuentes relacionadas con un mismo fenómeno numismático, notafílico o exonúmico. Procure que sean de naturaleza diferente y, cuando sea posible, que al menos una sea contemporánea al hecho estudiado.

Para cada una responda:

1. ¿Qué afirma?
2. ¿Qué podía conocer directamente quien la produjo?
3. ¿Cuál era su propósito o destinatario?
4. ¿Existe algún interés que deba considerarse?
5. ¿Las tres fuentes son realmente independientes?
6. ¿En qué coinciden?
7. ¿En qué se contradicen?
8. ¿Qué considera demostrado?
9. ¿Qué considera solamente probable o posible?
10. ¿Qué todavía no puede determinarse?
11. ¿Qué nueva fuente buscaría para aumentar la certeza?

La actividad es formativa. No se busca decidir cuál autor “gana”, sino aprender a determinar **qué permite afirmar cada evidencia**.
` }],
  },
  {
    titulo: "Trabajo escrito de la unidad 3",
    bloques: [{ tipo: "texto", contenido: `
Elabore una **nota crítica de fuentes de 500 a 750 palabras**.

Seleccione una afirmación numismática, notafílica o exonúmica encontrada durante su investigación y rastree su presencia en **al menos tres fuentes**.

El trabajo deberá incluir:

1. la afirmación investigada;
2. la fuente más antigua que logró localizar;
3. cómo aparece la afirmación en las demás fuentes;
4. posibles relaciones de dependencia entre ellas;
5. evaluación de autoría, fecha, propósito y proximidad;
6. coincidencias o contradicciones;
7. evidencia independiente disponible;
8. una conclusión propia, clasificada como **demostrada, probable, posible o no determinable con la evidencia disponible**;
9. la fuente adicional que sería necesaria para aumentar la certeza.

Incluya citas y referencias completas.

**El estudiante no será penalizado por concluir que algo no puede demostrarse. Sí será observado si afirma más de lo que sus fuentes permiten.**

Esta será la última nota breve del primer bloque metodológico. En unidad 4 aumentará la extensión y se aplicarán estas competencias al problema de la identificación, clasificación y catalogación.
` }],
  },
  {
    titulo: "Referencias de la unidad 3",
    bloques: [{ tipo: "texto", contenido: `
- Batthyány, K., & Cabrera, M. (Coords.). (2011). *Metodología de la investigación en Ciencias Sociales: Apuntes para un curso inicial*. Universidad de la República.
- Banco de Guatemala. (2012). *Del jade al polímero*. Banco de Guatemala / Ediciones OPP.
- Morales Osterberg, M. M. (2026). *Exonumia e Historia: Fichas particulares de Guatemala* (2.ª ed.). Exonumis Editores.
- Pérez Irungaray, G. E. (2025). *Notafilia guatemalteca: Un recorrido por la historia y el arte de los billetes de Guatemala*. Universidad Rafael Landívar, Editorial Cara Parens.
- Prober, K. (1973). *Historia numismática de Guatemala* (2.ª ed.).
- Quisquinay Rojas, E. D. (2022). Primeros billetes de banco en Guatemala: Banco Nacional de Guatemala (1874–1876). *Revista de Investigación Proyección Científica, 4*(1), 141–153.
- Sandoval Abullarade, V. H. (s. f.). *Monedas de Guatemala*.

**Nota metodológica:** estas obras cumplen funciones distintas: metodología general, fuentes institucionales, estudios especializados y obras de referencia. La pertinencia de una fuente siempre dependerá de la afirmación concreta que se pretenda sostener.
` }],
  },

  { titulo: "Profundización 1. La crítica externa e interna de una fuente", bloques: [{ tipo: "texto", contenido: `
La crítica puede comenzar por establecer autenticidad, fecha, autoría, procedencia e integridad del documento, y continuar examinando contenido, propósito, lenguaje y relación con otros testimonios. Howell y Prevenier (2001) desarrollan estos problemas como parte central del método histórico. En numismática, la crítica se extiende también al objeto: autenticidad, alteraciones, atribuciones y contexto de procedencia pueden condicionar su valor probatorio.

No toda investigación requerirá resolver todos estos aspectos, pero el estudiante debe reconocer cuáles afectan su pregunta. Una fuente auténtica puede contener información errónea; una fuente interesada puede conservar datos valiosos; una copia tardía puede transmitir un documento perdido. La crítica evita juicios binarios de “confiable/no confiable”.
` }] },
  { titulo: "Profundización 2. Autor, destinatario y propósito", bloques: [{ tipo: "texto", contenido: `
Preguntar quién produjo una fuente y para quién ayuda a interpretar qué información era importante registrar y qué podía omitirse. Un informe administrativo, un anuncio comercial, una memoria institucional y una noticia periodística responden a propósitos distintos. Bloch (1953) advierte que incluso testimonios involuntarios pueden proporcionar información que su autor nunca pretendió transmitir.

El investigador debe distinguir entre lo que la fuente quiso afirmar y lo que permite observar indirectamente. Una propaganda puede ser débil para describir objetivamente una situación, pero excelente para estudiar el mensaje que una institución intentaba difundir.
` }] },
  { titulo: "Profundización 3. Dependencia y genealogía de afirmaciones", bloques: [{ tipo: "texto", contenido: `
Una afirmación puede circular durante décadas y adquirir apariencia de hecho por repetición. La crítica exige rastrear, cuando sea posible, quién la formuló primero y qué evidencia utilizó. Howell y Prevenier (2001) muestran la importancia de reconstruir la transmisión del testimonio.

Si cinco autores dependen del mismo catálogo, no existen cinco confirmaciones independientes. El investigador debe buscar líneas de evidencia distintas: legislación, pieza, archivo, prensa o registros. La convergencia entre fuentes independientes fortalece una conclusión más que la repetición bibliográfica.
` }] },
  { titulo: "Profundización 4. Contradicciones y resolución de discrepancias", bloques: [{ tipo: "texto", contenido: `
Las discrepancias no deben resolverse eligiendo automáticamente la fuente más reciente o la que parezca más autorizada. Primero debe determinarse si las fuentes hablan realmente del mismo fenómeno. Una puede referirse a autorización y otra a circulación; una a fecha de fabricación y otra a fecha inscrita.

Cuando la contradicción persiste, debe exponerse. Booth et al. (2016) recomiendan anticipar objeciones y evidencia contraria como parte de un argumento responsable. Una conclusión que reconoce una discrepancia no es más débil: es más transparente respecto del estado del conocimiento.
` }] },
  { titulo: "Profundización 5. Incertidumbre y lenguaje académico", bloques: [{ tipo: "texto", contenido: `
El grado de certeza debe reflejar la calidad de la evidencia. “Demuestra”, “documenta”, “sugiere”, “es compatible con”, “podría” y “no puede determinarse” no son equivalentes. El investigador debe escogerlos según la relación real entre fuente y afirmación.

Reconocer incertidumbre no significa refugiarse siempre en expresiones vagas. Cuando la evidencia es directa y suficiente, corresponde formular conclusiones firmes y delimitadas. El rigor consiste en no afirmar menos ni más de lo que las fuentes permiten.
` }] },

  { titulo: "Profundización 6. Autenticidad no equivale a veracidad", bloques: [{ tipo: "texto", contenido: `
Comprobar que un documento es auténtico responde a una pregunta distinta de determinar si su contenido es verdadero. Una carta genuina puede contener rumores; un informe oficial auténtico puede seleccionar información favorable a la institución; una memoria escrita décadas después puede mezclar recuerdo y reconstrucción. Bloch (1953) insistió en la necesidad de interrogar los testimonios incluso cuando su autenticidad no está en duda.

En numismática ocurre algo semejante: una pieza auténtica demuestra la existencia de ese ejemplar y sus características materiales, pero no convierte automáticamente en verdaderas todas las historias de procedencia asociadas a ella. La cadena documental debe evaluarse por separado.

Esta distinción permite evitar dos errores opuestos: aceptar sin crítica todo documento oficial o descartar por completo una fuente parcial. La pregunta adecuada es qué información concreta puede sostenerse con ella y qué corroboración adicional necesita.
` }] },
  { titulo: "Profundización 7. Proximidad temporal y conocimiento efectivo", bloques: [{ tipo: "texto", contenido: `
Una fuente contemporánea a un acontecimiento puede ser valiosa, pero contemporaneidad no significa conocimiento directo. Un periódico publicado el mismo día puede repetir información de terceros; un funcionario puede desconocer decisiones tomadas en otra dependencia. Howell y Prevenier (2001) recomiendan examinar la posición del testigo respecto del hecho que describe.

Por ello conviene preguntar: ¿cómo pudo saberlo?, ¿estuvo presente?, ¿tuvo acceso a registros?, ¿reproduce otra fuente?, ¿escribe antes o después de conocer el resultado? Estas preguntas ayudan a valorar la proximidad epistemológica, no solo cronológica.

Una fuente posterior puede, en ciertos casos, disponer de archivos que un testigo contemporáneo no conocía. La comparación entre testimonios cercanos y reconstrucciones posteriores debe hacerse según la afirmación concreta que se intenta sostener.
` }] },
  { titulo: "Profundización 8. Corroborar sin contar votos", bloques: [{ tipo: "texto", contenido: `
La corroboración no funciona como una votación bibliográfica. Diez autores que repiten el mismo dato no superan automáticamente a un documento contemporáneo que lo contradice. El investigador debe identificar independencia, calidad y alcance de cada línea de evidencia.

Una estrategia consiste en construir una tabla con afirmación, fuente inmediata, fuente originaria y grado de independencia. Esto permite detectar cadenas de copia y distinguir consenso historiográfico de corroboración documental. Booth et al. (2016) señalan que un argumento fuerte debe responder a objeciones razonables; una fuente contradictoria de alta calidad no puede neutralizarse simplemente citando más obras dependientes.

Cuando la evidencia permanece dividida, la conclusión debe expresarlo. La tarea no es forzar unanimidad, sino describir con precisión por qué unas fuentes merecen mayor peso para la pregunta planteada.
` }] },
  { titulo: "Profundización 9. El silencio como evidencia condicionada", bloques: [{ tipo: "texto", contenido: `
El argumento del silencio solo adquiere fuerza cuando existe una razón fundada para esperar que la fuente registrara el fenómeno. Si un libro contable conserva sistemáticamente todas las emisiones de un período y una denominación nunca aparece, la ausencia puede ser informativa. Si la serie está incompleta, el silencio es mucho más débil.

El investigador debe documentar la expectativa de presencia: ¿qué función tenía el registro?, ¿qué años conserva?, ¿qué omisiones conocidas presenta?, ¿existen controles cruzados? Sin esta evaluación, “no aparece” se transforma indebidamente en “no existió”.

Bloch (1953) mostró que los silencios documentales forman parte de los problemas del historiador. En AGENN, el uso responsable de evidencia negativa exige declarar el corpus examinado y formular la conclusión de manera proporcional a su completitud.
` }] },
  { titulo: "Profundización 10. Construir una conclusión auditable", bloques: [{ tipo: "texto", contenido: `
Una conclusión sólida debe permitir reconstruir su camino probatorio. Para cada afirmación central, el investigador debería poder indicar qué fuentes la sostienen, qué fuentes la contradicen, qué inferencia realizó y qué grado de certeza asigna. Esta práctica convierte la crítica de fuentes en argumento y prepara las competencias que se profundizarán en las unidades 8 y 9.

Howell y Prevenier (2001) ofrecen herramientas para evaluar testimonios; Booth et al. (2016) ayudan a convertir evidencia en afirmaciones defendibles. La combinación resulta especialmente útil en investigación numismática, donde el objeto material y el documento escrito suelen responder preguntas diferentes.

Una conclusión auditable no necesita ser definitiva. Puede establecer que una hipótesis es probable, que dos explicaciones siguen abiertas o que la evidencia disponible no permite decidir. Lo importante es que el lector pueda comprender por qué se llegó exactamente hasta ese punto.
` }] },
];

export const QUESTIONS = [
  {
    pregunta: "Una fuente académica muy citada debe aceptarse como verdadera sin revisar la evidencia en que se basa.",
    tipo: "fv",
    opciones: ["Falso","Verdadero"], correcta: 0,
    explicacion: `
La autoridad o frecuencia de cita no sustituye la evaluación crítica de la evidencia.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Cuál es la pregunta central de la crítica de fuentes?",
    tipo: "mc",
    opciones: ["¿Cuántas páginas tiene?","¿Es la fuente más antigua disponible?","¿Qué me permite saber esta fuente y con qué grado de confianza?","¿Coincide con mi hipótesis?"], correcta: 2,
    explicacion: `
La crítica determina alcance y confiabilidad para una afirmación concreta.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "La proximidad temporal de una fuente garantiza que su contenido sea exacto.",
    tipo: "fv",
    opciones: ["Verdadero","Falso"], correcta: 1,
    explicacion: `
Un contemporáneo puede equivocarse, desconocer hechos o defender intereses.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Cuál conjunto de elementos ayuda a contextualizar una fuente?",
    tipo: "mc",
    opciones: ["Precio, tamaño de letra y encuadernación","Número de fotografías únicamente","Popularidad del autor","Autoría, fecha, propósito, destinatario y contexto"], correcta: 3,
    explicacion: `
Conocer quién produjo la fuente, cuándo y para qué ayuda a establecer su alcance.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "Una fuente primaria es siempre superior a una secundaria.",
    tipo: "fv",
    opciones: ["Falso","Verdadero"], correcta: 0,
    explicacion: `
Primaria y secundaria describen relaciones con el fenómeno, no una jerarquía automática de calidad.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Qué fuente sería más directa para demostrar qué ordenó legalmente una reforma monetaria?",
    tipo: "mc",
    opciones: ["El decreto correspondiente","Un catálogo moderno","Una publicación en redes sociales","El precio actual de las piezas"], correcta: 0,
    explicacion: `
La disposición normativa es evidencia directa de lo ordenado por la autoridad.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "Nueve libros que dependen de una misma afirmación original constituyen nueve corroboraciones independientes.",
    tipo: "fv",
    opciones: ["Verdadero","Falso"], correcta: 1,
    explicacion: `
Puede tratarse de una única tradición bibliográfica repetida.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Qué significa reconstruir la genealogía de una afirmación?",
    tipo: "mc",
    opciones: ["Buscar la biografía familiar del autor","Rastrear quién la formuló, cómo fue repetida y en qué evidencia se apoyaba","Contar las ediciones de un libro","Elegir la versión más reciente"], correcta: 1,
    explicacion: `
El objetivo es llegar al origen documental de la afirmación.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "Una fuente interesada debe descartarse automáticamente.",
    tipo: "fv",
    opciones: ["Falso","Verdadero"], correcta: 0,
    explicacion: `
El interés debe analizarse como parte del contexto y contrastarse con otras evidencias.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "Un propietario describe favorablemente un sistema que él mismo utiliza. ¿Qué procede?",
    tipo: "mc",
    opciones: ["Aceptar el relato por ser contemporáneo","Descartarlo sin leerlo","Analizar su posición y buscar evidencia independiente","Convertirlo en prueba de aceptación general"], correcta: 2,
    explicacion: `
Su posición no invalida el testimonio, pero condiciona su uso.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "Cuando dos fuentes se contradicen, debe elegirse la escrita por el autor más prestigioso.",
    tipo: "fv",
    opciones: ["Verdadero","Falso"], correcta: 1,
    explicacion: `
La contradicción debe analizarse mediante contexto, dependencia y evidencia adicional.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Cuál puede ser una conclusión legítima después de estudiar fuentes contradictorias?",
    tipo: "mc",
    opciones: ["Debe ser cierta la fuente más antigua","Ambas son necesariamente falsas","La contradicción debe omitirse","La evidencia disponible no permite determinarlo con certeza"], correcta: 3,
    explicacion: `
Reconocer incertidumbre es una conclusión académicamente válida.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "No encontrar una pieza demuestra que nunca existió.",
    tipo: "fv",
    opciones: ["Falso","Verdadero"], correcta: 0,
    explicacion: `
La ausencia de evidencia no equivale automáticamente a evidencia de ausencia.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Cuándo puede adquirir relevancia el silencio documental?",
    tipo: "mc",
    opciones: ["Cuando razonablemente se esperaba encontrar una huella en una serie adecuada y suficientemente completa","Siempre que no aparezca un resultado en Google","Cuando contradice nuestra hipótesis","Nunca"], correcta: 0,
    explicacion: `
La ausencia solo adquiere fuerza cuando puede justificarse la expectativa de evidencia.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "Un documento que afirma que ocurrió un hecho demuestra directamente, como mínimo, que alguien hizo esa afirmación.",
    tipo: "fv",
    opciones: ["Falso","Verdadero"], correcta: 1,
    explicacion: `
La verdad histórica de lo afirmado puede requerir evaluación y corroboración adicional.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Cuál es una inferencia?",
    tipo: "mc",
    opciones: ["La inscripción visible de una moneda","Una relación razonada construida a partir de evidencias que no la expresan directamente","La transcripción literal de un decreto","El título de un catálogo"], correcta: 1,
    explicacion: `
La inferencia conecta evidencias mediante razonamiento explícito.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "Una interpretación histórica y una observación material son exactamente el mismo nivel de afirmación.",
    tipo: "fv",
    opciones: ["Falso","Verdadero"], correcta: 0,
    explicacion: `
La interpretación integra y explica evidencias; la observación describe características directamente constatables.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Qué combinación ofrecería evidencia convergente sobre una emisión?",
    tipo: "mc",
    opciones: ["Cuatro catálogos que copian el mismo dato","Cuatro fotografías de la misma página","Decreto, registro de fabricación, ejemplar y prensa contemporánea","Cuatro opiniones sin fuente"], correcta: 2,
    explicacion: `
Fuentes de distinta naturaleza pueden documentar etapas diferentes del fenómeno.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "Cuanto más amplia sea una afirmación, normalmente mayor deberá ser la evidencia que la respalde.",
    tipo: "fv",
    opciones: ["Falso","Verdadero"], correcta: 1,
    explicacion: `
Generalizaciones amplias requieren una base empírica proporcional.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Cuál afirmación requiere menos evidencia?",
    tipo: "mc",
    opciones: ["La pieza circuló ampliamente en toda Guatemala","Toda la población la aceptó","Fue la moneda preferida durante décadas","Existe al menos un ejemplar documentado con esta característica"], correcta: 3,
    explicacion: `
Una afirmación existencial concreta tiene un alcance mucho menor.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "La palabra 'sugiere' puede ser más rigurosa que 'demuestra' cuando la evidencia es incompleta.",
    tipo: "fv",
    opciones: ["Verdadero","Falso"], correcta: 0,
    explicacion: `
El lenguaje debe reflejar el grado real de certeza.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Qué expresión corresponde cuando la evidencia no contradice una hipótesis pero tampoco la prueba?",
    tipo: "mc",
    opciones: ["Es compatible con","Demuestra","Confirma definitivamente","Es indudable"], correcta: 0,
    explicacion: `
Compatibilidad no equivale a demostración.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "Modificar una hipótesis frente a nueva evidencia constituye un fracaso del investigador.",
    tipo: "fv",
    opciones: ["Verdadero","Falso"], correcta: 1,
    explicacion: `
Revisar conclusiones demuestra que la investigación está gobernada por evidencia.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Qué debe hacer el estudiante si después de una búsqueda rigurosa no puede resolver una contradicción?",
    tipo: "mc",
    opciones: ["Inventar una solución probable y presentarla como hecho","Expresar claramente que no puede determinarse con la evidencia disponible","Omitir una de las fuentes","Elegir la explicación que más le guste"], correcta: 1,
    explicacion: `
Los límites documentales deben formar parte explícita de la conclusión.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
  {
    pregunta: "¿Cuál es la competencia central de U3?",
    tipo: "mc",
    opciones: ["Memorizar nombres de autores","Coleccionar el mayor número de referencias","Evaluar críticamente fuentes y ajustar las afirmaciones al alcance de la evidencia","Defender siempre la hipótesis inicial"], correcta: 2,
    explicacion: `
U3 enseña a construir evidencia mediante crítica, contraste y reconocimiento de incertidumbre.

**Para el investigador:** ajuste siempre la fuerza de su conclusión a la fuerza de la evidencia disponible.
`,
  },
];
