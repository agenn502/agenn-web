...
export async function GET(req: NextRequest) {
  try {
    ...
    // La portada pública usa esta respuesta mínima para ofrecer al CA el
    // acceso al número provisional sin anunciarlo en el archivo público.
    if (req.nextUrl.searchParams.get("acceso_prueba") === "1") {
      const { data: numeroPrueba, error: numeroPruebaError } =
      ...
      return NextResponse.json({
        ...
        numero_prueba: numeroPrueba
          ? {
              id: Number(numeroPrueba.id),
              slug: String(numeroPrueba.slug || ""),
              titulo: String(numeroPrueba.titulo || "Prueba"),
            }
          : null,
      });
    }
    ...
    const autorIds = [
      ...new Set(
        lista.map((m: any) => Number(m.autor_miembro_id)).filter(Boolean),
      ),
    ];
    ...
    const autoresPorId = new Map(
      autores.map((autor: any) => [Number(autor.id), autor]),
    );
    ...
    for (const version of versiones) {
      ...
      if (!versionPorManuscrito.has(id)) {
        versionPorManuscrito.set(id, version);
      }
    }
