...
const obtenerPerfil = cache(async (codigo: string): Promise<PerfilAutor | null> => {
  ...
  const { data: articulos, error: articulosError } = await supabaseServer
    ...
    .in("manuscrito_id", manuscritos.map((manuscrito) => manuscrito.id));

  if (articulosError) throw new Error(articulosError.message);
  if (!articulos?.length) return null;

  const revistaIds = [...new Set(articulos.map((articulo) => Number(articulo.revista_id)))];
  const versionIds = [...new Set(articulos.map((articulo) => Number(articulo.version_id)))];

  const [{ data: revistas, error: revistasError }, { data: versiones, error: versionesError }] =
    await Promise.all([
      supabaseServer
        .from("revistas")
  ...
  const revistaPorId = new Map(revistas.map((revista) => [Number(revista.id), revista]));
  const versionPorId = new Map((versiones || []).map((version) => [Number(version.id), version]));
    ...
    <div className={styles.pagina}>
      ...
      <section className={styles.perfil}>
        ...
        <div className={styles.datos}>
          ...
          {autor.profesion && <p className={styles.profesion}>{autor.profesion}</p>}

          <div className={styles.biografia}>
            <h2>Biografía</h2>
            <p>
              {autor.bio || "La biografía de este autor se encuentra en preparación."}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className={styles.trabajos}>
        ...
        {trabajos.map((trabajo) => (
          <article key={trabajo.id}>
            ...
            <div>
              <h3>
                <Link href={`/revista/${trabajo.anio}/${trabajo.numero}/${trabajo.localizador}`}>
                  {trabajo.titulo}
                </Link>
              </h3>
              <p>
                Revista AGENN · Vol. {trabajo.volumen || "—"} · Núm. {trabajo.numero} · {trabajo.anio}
              </p>
            </div>
          </article>
        ))}
