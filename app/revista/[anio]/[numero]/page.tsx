...
export async function generateMetadata({
  ...
  params: Promise<{ anio: string; numero: string }>;
}): Promise<Metadata> {
  const parametros = await params;
  const anio = Number(parametros.anio);
  const numeroValor = Number(parametros.numero);
  const numero =
    Number.isInteger(anio) && Number.isInteger(numeroValor)
      ? await obtenerNumero(anio, numeroValor)
      : null;
...
export default async function NumeroPublicadoPage({
  ...
  if (!Number.isInteger(anio) || !Number.isInteger(numeroValor)) notFound();

  const numero = await obtenerNumero(anio, numeroValor);
      ...
      <header className={styles.cabeceraNumero}>
        {numero.portada_url && (
          <div className={styles.portada}>
            <img src={numero.portada_url} alt="Portada de este número" />
          </div>
        )}
