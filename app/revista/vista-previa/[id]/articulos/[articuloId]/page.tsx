...
function renderInline(texto: string): ReactNode[] {
  ...
  partesFormato.forEach((parte) => {
    if (parte.startsWith("**") && parte.endsWith("**")) {
      ...
      salida.push(<strong key={`strong-${clave++}`}>{contenido}</strong>);
      return;
    }
...
function figuraHtml(imagen: Imagen) {
  ...
  return `<figure style="margin:2rem auto 1.25rem;max-width:760px;text-align:center"><img src="${escaparHtml(imagen.url)}" alt="${escaparHtml(imagen.titulo || "Imagen del artículo")}" style="width:100%;height:auto;display:block;border-radius:4px" />${titulo || fuente ? `<figcaption style="margin-top:.65rem;font-family:Arial,sans-serif;font-size:14px;line-height:1.5;color:#555">${titulo}${fuente}</figcaption>` : ""}</figure>`;
}
...
function convertirUrlsHtmlEnEnlaces(html: string) {
  ...
  nodos.forEach((nodo) => {
    ...
    while ((coincidencia = regexUrl.exec(texto))) {
      ...
      enlace.className = "enlace-contenido";
...
function htmlEnriquecidoConImagenes(contenido: string, imagenes: Imagen[]) {
  ...
  return convertirUrlsHtmlEnEnlaces(
    eliminarBloquesVaciosTrasFiguras(htmlConImagenes),
  );
}
  ...
  return partes.map((parte, indice) => {
    ...
    if (match) {
      ...
      return (
        <figure
          key={indice}
          style={{ margin: "2rem auto 1.25rem", maxWidth: "760px" }}
        >
          <img
            src={imagen.url}
  ...
  return (
    ...
    >
      <nav style={{ marginBottom: "2rem" }}>
        <Link
          href={`/revista/vista-previa/${numeroId}`}
          style={{
            color: "#4d371c",
            fontWeight: 700,
            textDecoration: "none",
      ...
      <style jsx global>{`
        ...
        .contenido-html-enriquecido figure + h2,
        .contenido-html-enriquecido figure + h3 {
          margin-top: 1.25rem;
        }
