...
const PERFILES: Record<string, string> = {
  A: "Comerciante profesional", A2: "Comerciante con orientaciÃ³n acadÃ©mica",
  B: "Operador comercial no Ã©tico de alto conocimiento", B2: "Operador de doble rol acadÃ©mico-comercial no Ã©tico",
  C: "Comerciante en formaciÃ³n", D: "Vendedor no especializado", E: "NumismÃ¡tico especializado",
  F: "Investigador de enfoque amplio", G: "Coleccionista orientado a la calidad",
  H: "Coleccionista orientado a la completitud", I: "Coleccionista en desarrollo", J: "Acumulador no sistemÃ¡tico",
};
const INTERESES = ["Monedas","Billetes","Monedas y billetes","Medallas","Fichas","Condecoraciones","Historia monetaria","Otro"];
const NIVELES = ["EducaciÃ³n primaria","EducaciÃ³n media","Diversificado","Estudiante universitario","TÃ©cnico universitario","Licenciatura","MaestrÃ­a","Doctorado","Otro"];
...
async function comprimirFotografia(file: File): Promise<File> {
  ...
  const convertir = (calidad: number) => new Promise<Blob>((resolve, reject) => { const c = document.createElement("canvas"); c.width = ancho; c.height = alto; const x = c.getContext("2d"); if (!x) return reject(new Error("No fue posible preparar la fotografÃ­a.")); x.fillStyle = "#fff"; x.fillRect(0,0,ancho,alto); x.drawImage(imagen,0,0,ancho,alto); c.toBlob((b) => b ? resolve(b) : reject(new Error("No fue posible procesar la fotografÃ­a.")), "image/jpeg", calidad); });
  ...
  if (blob.size > FOTO_MAX_BYTES) throw new Error("No fue posible procesar la fotografÃ­a seleccionada. Intente con otra imagen.");
...
function CorregirSolicitudContenido() {
  ...
  return <section className="section"><div className="container content-page" style={{maxWidth:980}}><h1>CorrecciÃ³n de solicitud {datos.codigo}</h1>
    <div style={{...bloque,background:"#fff8e5"}}><strong>Observaciones del Consejo AcadÃ©mico</strong><p style={{whiteSpace:"pre-wrap"}}>{datos.observaciones_candidato}</p></div>
    <form onSubmit={enviar}>
      <div style={bloque}><h2>Perfil y datos personales</h2><label>Perfil<select name="perfil_codigo" value={datos.perfil_codigo} onChange={e=>set("perfil_codigo",e.target.value)} style={campo}>{Object.entries(PERFILES).map(([c,n])=><option key={c} value={c}>Perfil {c} â€” {n}</option>)}</select></label>
      <div style={{display:"grid",gap:"1rem",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",marginTop:"1rem"}}>
      {[['nombres','Nombres'],['apellidos','Apellidos'],['fecha_nacimiento','Fecha de nacimiento'],['correo','Correo electrÃ³nico'],['telefono','TelÃ©fono o WhatsApp'],['profesion_oficio','ProfesiÃ³n u oficio'],['institucion_trabajo','InstituciÃ³n o trabajo'],['departamento','Departamento'],['municipio','Municipio'],['comunidad','Comunidad, zona o barrio']].map(([k,l])=><label key={k}>{l}<input type={k==='fecha_nacimiento'?'date':k==='correo'?'email':'text'} name={k} value={datos[k] || ''} onChange={e=>set(k,e.target.value)} required={['nombres','apellidos','fecha_nacimiento','correo','profesion_oficio','departamento','municipio'].includes(k)} style={campo}/></label>)}
      <label>Nivel acadÃ©mico<select name="nivel_academico" value={datos.nivel_academico || ''} onChange={e=>set('nivel_academico',e.target.value)} required style={campo}>{NIVELES.map(n=><option key={n}>{n}</option>)}</select></label></div>
      <label style={{display:"block",marginTop:"1rem"}}>Reemplazar fotografÃ­a (opcional)<input type="file" accept="image/jpeg,image/png,image/webp" style={campo} onChange={async e=>{const f=e.target.files?.[0]; if(!f)return; try{const c=await comprimirFotografia(f); setFoto(c); setFotoPreview(URL.createObjectURL(c));}catch(err){setError(err instanceof Error?err.message:"No fue posible procesar la fotografÃ­a.");}}}/></label>{fotoPreview&&<img src={fotoPreview} alt="FotografÃ­a" style={{width:150,height:150,objectFit:"cover",borderRadius:10,marginTop:"1rem"}}/>}</div>
      ...
      <div style={{display:"grid",gap:"1rem",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",marginTop:"1rem"}}><label>Otro interÃ©s<input name="interes_otro" value={datos.interes_otro||''} onChange={e=>set('interes_otro',e.target.value)} style={campo}/></label><label>AÃ±os de experiencia<input type="number" min="0" name="experiencia_anios" value={datos.experiencia_anios??''} onChange={e=>set('experiencia_anios',e.target.value)} style={campo}/></label><label>Â¿Posee colecciÃ³n?<select value={String(datos.posee_coleccion)} onChange={e=>set('posee_coleccion',e.target.value==='true')} style={campo}><option value="true">SÃ­</option><option value="false">No</option></select></label><label>Â¿Participa en comunidad?<select value={String(datos.participa_comunidad)} onChange={e=>set('participa_comunidad',e.target.value==='true')} style={campo}><option value="true">SÃ­</option><option value="false">No</option></select></label></div>
      {[['comunidad_numismatica','AsociaciÃ³n, grupo o comunidad'],['areas_interes','Ãreas o piezas de interÃ©s'],['como_conocio_agenn','Â¿CÃ³mo conociÃ³ la AGENN?'],['motivacion','Â¿Por quÃ© desea ingresar?'],['expectativas_aprendizaje','Â¿QuÃ© espera aprender?']].map(([k,l])=><label key={k} style={{display:"block",marginTop:"1rem"}}>{l}<textarea name={k} value={datos[k]||''} onChange={e=>set(k,e.target.value)} required={['como_conocio_agenn','motivacion'].includes(k)} rows={k==='motivacion'?6:4} style={campo}/></label>)}</div>
