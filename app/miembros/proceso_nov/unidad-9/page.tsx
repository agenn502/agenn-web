...
import { TEORIA, QUESTIONS } from "@/content/proceso_nov/unidad9";
...
export default function Unidad9NovicioPage() {
  ...
  const guardarAvanceParcial = async (preguntasCompletadas: number) => {
    ...
    if (error) {
      console.error("Error guardando avance parcial de NOV U9:", error);
      return false;
    }
  ...
  const guardarProgresoCuestionario = async () => {
    ...
    if (error) {
      console.error("Error guardando progreso de NOV U9:", error);
      setErrorGuardado(
  ...
  useEffect(() => {
    const cargarProgreso = async () => {
      ...
      if (error) {
        console.error("Error cargando progreso de NOV U9:", error);
      }
  ...
  useEffect(() => {
    ...
    const claveMemoria = `agenn:lectura:nov:unidad-9:${codigo}`;
  ...
  return (
    <div style={{ maxWidth: "980px" }}>
      ...
      <h1 style={{ marginTop: 0 }}>
        Unidad 9: Historia y análisis del papel moneda guatemalteco
      </h1>
        ...
        {completado && !modoRepaso && (
          ...
          >
            ...
            <p style={{ lineHeight: 1.8 }}>
              ...
              La Unidad 9 ha quedado registrada como finalizada y puede continuar
