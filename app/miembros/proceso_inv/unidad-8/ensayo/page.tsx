...
const REGLA_UNIDAD = obtenerReglaTrabajoInv("unidad-8")!;
const TEMAS_UNIDAD = TEMAS_TRABAJOS_INV["unidad-8"] || [];
...
export default function TrabajoUnidad1Page() {
  ...
  const generarCodigoVerificacion = () => {
    ...
    return `AGENN-INV-U8-${miembro?.codigo || "USER"}-${random}`;
  };
  ...
  const subirImagen = async () => {
    ...
    const nombreArchivo =
      `unidad-8/${miembro.codigo}-u8.jpg`;
  ...
  const guardarTrabajo = async (enviarRevision = false) => {
    ...
    try {
      ...
      const slug =
        slugTrabajo ||
        `${generarSlug(titulo)}-${miembro.codigo.toLowerCase()}-u8`;
  ...
  const solicitarRevisionPreliminar = async () => {
    ...
    try {
      ...
      const respuesta = await fetch("/api/inv/revision-preliminar", {
        ...
        body: JSON.stringify({
          unidad: "Unidad 8",
          tipoTrabajo: REGLA_UNIDAD.tipo || "Ensayo académico",
          tema,
          titulo: titulo.trim(),
          consigna: `Elabore un ensayo académico basado en uno de los temas propuestos de la Unidad 8. El trabajo debe desarrollar el tema con claridad, argumentación y sustento, y cumplir al menos ${REGLA_UNIDAD.palabrasMinimas.toLocaleString("es-GT")} palabras y ${REGLA_UNIDAD.caracteresMinimos.toLocaleString("es-GT")} caracteres.`,
          ...
          requisitosCuantitativos: `Mínimo ${REGLA_UNIDAD.palabrasMinimas.toLocaleString("es-GT")} palabras y ${REGLA_UNIDAD.caracteresMinimos.toLocaleString("es-GT")} caracteres.`,
  ...
  if (unidadCompletada || aprobadoAcademicamente) {
    return (
      <div style={{ maxWidth: "900px" }}>
        <h1>Ensayo académico de la Unidad 8</h1>
        ...
        >
          ...
          >
            ...
            La Unidad 8 ha sido completada y puede continuar
