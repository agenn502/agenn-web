...
const REGLA_UNIDAD = obtenerReglaTrabajoInv("unidad-10")!;
const TEMAS_UNIDAD = TEMAS_TRABAJOS_INV["unidad-10"] || [];
...
export default function TrabajoUnidad1Page() {
  ...
  const generarCodigoVerificacion = () => {
    ...
    return `AGENN-INV-U10-${miembro?.codigo || "USER"}-${random}`;
  };
  ...
  const subirImagen = async () => {
    ...
    const nombreArchivo =
      `unidad-10/${miembro.codigo}-u10.jpg`;
  ...
  const guardarTrabajo = async (enviarRevision = false) => {
    ...
    try {
      ...
      const slug =
        slugTrabajo ||
        `${generarSlug(titulo)}-${miembro.codigo.toLowerCase()}-u10`;
  ...
  if (unidadCompletada || aprobadoAcademicamente) {
    return (
      <div style={{ maxWidth: "900px" }}>
        <h1>Ensayo académico de la Unidad 10</h1>
        ...
        >
          ...
          >
            ...
            La Unidad 10 ha sido completada y puede continuar
