...
const REGLA_UNIDAD = obtenerReglaTrabajoInv("unidad-7")!;
const TEMAS_UNIDAD = TEMAS_TRABAJOS_INV["unidad-7"] || [];
...
export default function TrabajoUnidad1Page() {
  ...
  const generarCodigoVerificacion = () => {
    ...
    return `AGENN-INV-U7-${miembro?.codigo || "USER"}-${random}`;
  };
  ...
  const subirImagen = async () => {
    ...
    const nombreArchivo =
      `unidad-7/${miembro.codigo}-u7.jpg`;
  ...
  const guardarTrabajo = async (enviarRevision = false) => {
    ...
    try {
      ...
      const slug =
        slugTrabajo ||
        `${generarSlug(titulo)}-${miembro.codigo.toLowerCase()}-u7`;
  ...
  if (unidadCompletada || aprobadoAcademicamente) {
    return (
      <div style={{ maxWidth: "900px" }}>
        <h1>Ensayo académico de la Unidad 7</h1>
        ...
        >
          ...
          >
            ...
            La Unidad 7 ha sido completada y puede continuar
