...
const REGLA_UNIDAD = obtenerReglaTrabajoInv("unidad-3")!;
const TEMAS_UNIDAD = TEMAS_TRABAJOS_INV["unidad-3"] || [];
...
export default function TrabajoUnidad1Page() {
  ...
  const generarCodigoVerificacion = () => {
    ...
    return `AGENN-INV-U3-${miembro?.codigo || "USER"}-${random}`;
  };
  ...
  const subirImagen = async () => {
    ...
    const nombreArchivo =
      `unidad-3/${miembro.codigo}-u3.jpg`;
  ...
  const guardarTrabajo = async (enviarRevision = false) => {
    ...
    try {
      ...
      const slug =
        slugTrabajo ||
        `${generarSlug(titulo)}-${miembro.codigo.toLowerCase()}-u3`;
  ...
  if (unidadCompletada || aprobadoAcademicamente) {
    return (
      <div style={{ maxWidth: "900px" }}>
        <h1>Análisis breve de la Unidad 3</h1>
        ...
        >
          ...
          >
            ...
            La Unidad 3 ha sido completada y puede continuar
