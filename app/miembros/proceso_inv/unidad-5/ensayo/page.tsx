...
const REGLA_UNIDAD = obtenerReglaTrabajoInv("unidad-5")!;
const TEMAS_UNIDAD = TEMAS_TRABAJOS_INV["unidad-5"] || [];
...
export default function TrabajoUnidad5Page() {
  ...
  const generarCodigoVerificacion = () => {
    ...
    return `AGENN-INV-U5-${miembro?.codigo || "USER"}-${random}`;
  };
  ...
  const subirImagen = async () => {
    ...
    const nombreArchivo =
      `unidad-5/${miembro.codigo}-u5.jpg`;
  ...
  const guardarTrabajo = async (enviarRevision = false) => {
    ...
    try {
      ...
      const slug =
        slugTrabajo ||
        `${generarSlug(titulo)}-${miembro.codigo.toLowerCase()}-u5`;
  ...
  if (unidadCompletada || aprobadoAcademicamente) {
    return (
      <div style={{ maxWidth: "900px" }}>
        <h1>Nota de investigación de la Unidad 5</h1>
        ...
        >
          ...
          >
            ...
            La Unidad 5 ha sido completada y puede continuar
